...
class BottomUpAffine : public transform::Transform {
 public:
  explicit BottomUpAffine(const Value& args) noexcept {
    assert(args.contains("image_size"));  // the value is `input_size` acrually
    from_value(args["image_size"], image_size_);
    crop_resize_pad_ =
        ::mmdeploy::operation::Managed<::mmdeploy::operation::CropResizePad>::Create();
  }

  ~BottomUpAffine() override = default;

  Result<void> Apply(Value& data) override {
    MMDEPLOY_DEBUG("bottom_up_affine input: {}", data);
    ...
    Tensor dst;
    {
      // s[0] *= 200;
      // s[1] *= 200;
      c[0] = (int)image_size_[0] / 2;
      c[1] = (int)image_size_[1] / 2;
      ...
      c[1] = c_src_y + v_src_y;
      // corrected scale
      s[0] = image_size_[0] / factor_x ;
      s[1] = image_size_[1] / factor_y ;
...
MMDEPLOY_REGISTER_TRANSFORM(BottomUpAffine);
